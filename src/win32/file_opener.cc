...
namespace moderna::io {
  ...
  */
  export template <open_mode mode> auto open_file(const fs::path &p, bool auto_create = true) {
    ...
    //   return generic_file<file_type, is_readable, is_writable, true, true,
    //   true>{std::move(file).fd(
    //   )};
    // });
  }
  ...
  */
  ...
  export std::expected<tcp_host<file_type>, fs_error> listen(
    int port, int backlog, bool non_blocking = true
  ) {
    ...
    //   return std::unexpected{fs_error::make(err_no, strerror(err_no))};
    // }
    // auto host = tcp_host<file_type>{.file{std::move(controlled_fd)}, .host{std::string{ip},
    // port}}; if (!non_blocking) {
  ...
  export std::expected<tcp_connection<file_type>, fs_error> connect(
    std::string_view ip, int port, bool non_blocking
  ) {
    // int fd = ::socket(AF_INET, SOCK_STREAM, 0);
    // int err_no = errno;
    // if (fd == -1) {
