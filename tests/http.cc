...
struct in_mem_file {
  ...
  in_mem_file(range_type r) : beg{std::ranges::cbegin(r)}, end{std::ranges::cend(r)} {}
  template <size_t N>
  std::expected<void, io::io_error> read(generic::fixed_string<N> &buf) noexcept {
    size_t i = 0;
    for (i = 0; i != N && beg != end; i += 1) {
      *(buf.begin() + i) = *beg;
      beg++;
    }
    buf.unsafe_set_length(i);
    return {};
  }
};
...
JOWI_ADD_TEST(test_valid_http_combo) {
  for (const auto &h : valid_headers) {
    auto [name, value] =
      test_lib::assert_expected_value(io::http::http_header::validate_header_line(std::get<0>(h)));
...
JOWI_ADD_TEST(test_http_header_multiline) {
  ...
  auto header =
    test_lib::assert_expected_value(io::http::http_header::validate_header_lines(header_str));
  test_lib::assert_equal(header.size(), valid_headers.size());
  for (const auto &h : valid_headers) {
    auto vs = header.filter(std::get<1>(h));
    test_lib::assert_false(vs.empty());
